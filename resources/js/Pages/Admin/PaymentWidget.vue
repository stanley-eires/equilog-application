...
const form = useForm( {
    bank_transfer: {
        transfer_details: props.payment?.bank_transfer.transfer_details ?? `<h4>Equilog Training & Consultancy</h4><ul class='list-unstyled'><li>Access Bank: <strong>0051030986</strong></li><li>Covenant Microfinance: <strong>0520058031</strong></li></ul>`,
        manual_payment_instructions: props.payment?.bank_transfer.manual_payment_instructions ?? ` <p>You can make payment via bank deposit or mobile transer directly to any of the bank account displayed in the invoice <strong>using the generated invoice code</strong> as narration. This makes it easier for the administrator to track the payment</p><p>You are to then click on the "Upload Payment Evidence" button below to upload evidence of successful payment. Proof of payment might consist of the bank teller used in making deposit or the receipt generated by your bank application and <strong>not your SMS debit notification.</strong> </p><p>Your invoice status chould change on successful verification or contact support if don't get anyresponse within the next 6 hours.</p>`
    },
    paystack: {
        status: props.payment?.paystack.status ?? true,
        live: props.payment?.paystack.live ?? false,
        pk_test: props.payment?.paystack.test_pk ?? 'pk_test_a509cf0b75f217f6e0adf29ab8cf622d86a70544',
        sk_test: props.payment?.paystack.sk_test ?? 'sk_test_1c22c679496097ff2a4eaffc8cd24c95ff27bab1',
        pk_live: props.payment?.paystack.pk_live ?? '',
        sk_live: props.payment?.paystack.sk_live ?? '',
    }
} )
...
<template>
    <div>
        ...
        <button @click.prevent="handleSave" class="btn btn-primary">Save Changes</button>
    </div>
</template>